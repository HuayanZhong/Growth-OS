---
alwaysApply: false
description: Backend test commands rule (pnpm + Jest): pnpm --filter server test for unit; test:e2e for e2e; verification order: test → typecheck → lint; CI skips env validation. Use when running tests or verifying before commits.
---
...
1. **Unit tests**: `pnpm --filter server test` (jest run, rootDir is `src/`, matches `**/*.spec.ts`).
...
7. **CI env**: `process.env.CI === 'true'` skips env validation (`validate()` returns config unchanged). Test accounts (`SUPABASE_TEST_EMAIL`, `SUPABASE_TEST_PASSWORD`) are in root `.env`, auto-loaded by vitest.config and jest.

**Example**:

```bash
# Single module test
pnpm --filter server test -- --testPathPattern=health
