---
alwaysApply: false
description: Backend test structure rule (Jest + NestJS): src/**/*.spec.ts for unit tests co-located with source; test/*.e2e-spec.ts for e2e tests; kebab-case files; one spec per module. Use when adding test files or choosing a directory.
---
...
3. **Configuration**: centralized in `package.json` `"jest"` section (`rootDir: "src"`, `testRegex: ".*\\.spec\\.ts$"`) + `test/jest-e2e.json` for e2e. Sub-modules don't scatter their own config.
