...
const getWeather = tool(async ({ location }) => `Weather in ${location}: Sunny, 72F`, {
  name: 'get_weather',
...
const search = tool(async ({ query }) => webSearch(query), {
  name: 'search',
  description:
    'Search the web for current information about a topic. Use this when you need recent data or facts.',
  schema: z.object({
