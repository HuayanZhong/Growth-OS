...
| `docs/desktop/architecture.md` (+ `.zh.md`) | Desktop frontend & shell detail doc; kept in place, linked from the map                                                                       | Live behavior that the map must carry                                           |
| `docs/i18n/`                                | Bilingual pairing contract (`README.md`) + terminology (`terminology.md`), hash-checked by `verify:docs` via `verify-translation-pairing.cjs` | Restated rule text, translation guidance                                        |
...
- **Pairs update together.** A Chinese mirror (`.zh.md`) changes in the same change as its English source; re-record the pair hash with `pnpm verify:pairing --write <path>`. `docs/guide-zh.md` re-records the index when structure moves. Mechanism and terminology: [docs/i18n/README.md](i18n/README.md), [terminology.md](i18n/terminology.md).
