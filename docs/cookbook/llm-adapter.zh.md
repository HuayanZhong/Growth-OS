...
- `LLMMessage` 是契约内独立的消息形状（`LLMMessageRole` + 可选工具调用字段）——调用方从自己域的历史存储组装 `messages: LLMMessage[]`。
...
注册到消费方的 module `providers`，或由它 import 一个独立的 `llm.module.ts`：
