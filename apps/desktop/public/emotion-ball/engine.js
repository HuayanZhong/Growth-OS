...
;(function () {
  ...
  var BOUNCE_TOTAL = BOUNCE_SEGS.reduce(function (s, q) {
    return s + q.d
  }, 0)
  ...
  function validate(raw) {
    ...
    if (raw.anims != null) {
      if (!Array.isArray(raw.anims)) errs.push('anims 必须是数组')
        ...
        raw.anims.forEach(function (a, i) {
          if (!a || !ANIM_TYPES[a.type])
            errs.push('anims[' + i + '] 未知动画类型：' + (a && a.type))
        })
    }
  ...
  function normalize(raw) {
    ...
    var pool = (raw.pool || [0, 8]).filter(function (i) {
      return i >= 0 && i < EXPR.length
    })
    ...
    var def = {
      ...
      antics: !!raw.antics,
      base: base,
      anims: (raw.anims || []).map(function (a) {
    ...
    if (raw.sequence) {
      var frames = raw.sequence.frames
        .map(function (f) {
  ...
  EB.config = {
    ...
    groups: function () {
      return GROUPS.map(function (g) {
        return { key: g.key, name: g.name, en: g.en || g.name }
      })
    },
  ...
  Engine.prototype = {
    ...
    setEmotion: function (id, o) {
      ...
      if (!def) {
        console.warn(
          '[EmotionBall] 未知表情 ID "' + id + '"，回退到待机 (' + this._fallbackId + ')',
        )
    ...
    registerEmotion: function (raw) {
      return EB.config.register(raw)
    },
    ...
    _compose: function (now, depth) {
      ...
      if (this._active && now >= this._poolNext) {
        if (def.pool.length > 1) {
          this._poolPos =
            (this._poolPos + 1 + Math.floor(rand(0, def.pool.length - 1))) % def.pool.length
