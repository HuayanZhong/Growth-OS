    ...
    function buildTrail(pts, width) {
      ...
      function seg(a, b) {
        var s = '',
          k
        for (k = a; k <= b; k++)
          s += (k === a ? 'M' : 'L') + r2(pts[k].x + nx[k]) + ' ' + r2(pts[k].y + ny[k])
        s += b === n - 1 ? cap(b) : 'L'
        for (k = b; k >= a; k--)
          s += (k === b ? '' : 'L') + r2(pts[k].x - nx[k]) + ' ' + r2(pts[k].y - ny[k])
