...
- Single entry point `pnpm hygiene` = `knip && turbo run publint`. knip runs once over the whole pnpm workspace (root `knip.json` with per-workspace `entry`/`ignoreDependencies`; framework-invoked deps such as `daisyui`/`tailwindcss` in CSS, `vue-tsc` via `nuxt typecheck`, `pino-pretty` as a pino transport string are ignored with a reason, not deleted — `tsx` left the ignore list when the server `openapi:export` script started using it). publint runs per library package (`shared`/`types`/`ui`) as a turbo task so it parallelizes in the existing graph. `desktop-core` is excluded: its entry is a build artifact (`dist/main.js`, produced by the desktop app's electron bundling) that does not exist in a fresh checkout, so the publish-correctness check cannot run there for a private, never-published package.
