...
- 静态扫描豁免（knip/oxlint 均看不到运行时按路径注入的引用）：`.oxlintrc.json` `ignorePatterns` 与 `knip.json` desktop 工作区 `ignore` 均加入 `**/public/emotion-ball/**`（或 `public/emotion-ball/**`），上游代码不做风格化修改。
