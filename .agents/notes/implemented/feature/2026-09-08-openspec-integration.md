...
- **跨平台共享**：`openspec/` 是纯 Markdown 规格库，任何平台读同一份；其他工具执行 `openspec init --tools <id>`（Claude/Cursor/Codex 等 30+）生成各自命令文件，规格与归档历史共享。共享 `.agents/skills/` 为 vendor-neutral 选项（本仓已有 `.agents/` 结构，天然兼容）。
