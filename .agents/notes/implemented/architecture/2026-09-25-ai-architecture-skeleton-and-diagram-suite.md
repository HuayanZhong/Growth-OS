...
**旧产物清理落地（2026-09-26，用户指令"先清理掉旧的"）**：探索实施路径时用户判定"设计定稿前的旧决定需清理重想"，落地三项：① 删除作废总方案 `.trae/documents/ai-module-plan.md`（doc-budgets manifest 条目、auth-verification-design.md 与两个 2026-08-25 note 的链接同步修复，verify 三段全绿）；② 删除空壳 change `rebuild-sessions-domain`（无 proposal，specs 下 4 个能力目录全空）；③ **`packages/types/src/adapters/llm.ts` 处置延后**——清理清单初判"删除"经全仓库引用扫描否决：它是 teardown 时显式保留的平台级契约（2026-09-13 design 原文）+ 双语 cookbook（llm-adapter.md/.zh.md）主题 + 4 张 archify 详图的 repository-evidence 锚点，误删将挂 validate/source-required。其去留归实施第一刀（立 ChatStreamEvent 契约）时的设计决策：改造为 provider-registry 接口或删除由 langchain 抽象接管，同变更带上 cookbook 与图证据。归零重推导的渐进切刀框架（S 立骨/H1-H3 加厚/L1-L2 通电，替代 tech-spec §4 的 P0-P4/M2-M5 引用）已经用户通过骨架落盘确认采纳（见下节），tech-spec §4 重写仍待做。

**AI 域骨架目录落盘（2026-09-26，change scaffold-ai-domain-skeleton；同日两次修正）**：结构演进三版——①按融合图七分区建 31 目录复合树（否决：违背仓库惯例）；②`modules/ai/` 单层聚合（否决：聚合容器本身也是别的模块没有的形态）；③**最终：AI 各域平级散落 `modules/` 下，一个域一个单层模块，与 auth/audit 同构，无聚合容器**。已建：`chat/`（SSE 聊天面）、`graph/`（编排引擎，langgraph/deepagentsjs 全仓唯一收口）、`model-provider/`（适配器工厂+注册表+binding+usage-meter，全平铺）+ `a2a/`（P3）+ `packages/types/src/ai/`（契约）+ `apps/desktop/app/components/chat/`。未来模块随各自刀建目录：session[H1]、agent-config[H2]、intent/knowledge[H3]、observability[L1]、memory/rsi[L2]。每目录一行 README = 职责+刀次+详图。同轮结构决策：两个家不建 ai-core 包（包提取时机 = 第二个真实消费者出现时）；SSE 帧编解码 S 刀进 `types/src/ai/chat-stream.ts`；`verify-invariants` 待补"langchain 系 import 只准出现在 modules/graph/**"门禁（随 S 刀）。教训：**结构落盘以仓库既有惯例为最高约束；"别的模块都没有"就是否决理由**。
