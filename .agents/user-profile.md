...
- 先评估再动手：升级/迁移/选型类任务先给官方依据与对比结论，经确认后执行。
  confidence: 3 ｜ evidence: 2026-09-08 ESM/rspack 评估先报告后执行 ｜ last_updated: 2026-09-08
...
- 重建/设计类任务先映射既有设计记录：动手前读全部相关 Notes 与归档 changes，产出新旧概念映射，不另起炉灶。
...
- 实施反对一次到位：大任务须渐进切刀（骨架→加厚→闭环），每刀一个风险源、独立验收；设计定稿后回头清理设计前的旧结论，不默认沿用。
  confidence: 3 ｜ evidence: 2026-09-26 "一次到位会出现很多bug"；"先删掉以前的决定重新思考"；骨架先看效果零实现；连续追问结构粒度收敛最小 workspace ｜ last_updated: 2026-09-26
...
- 偏好统一机制而非双轨：倾向把并行机制合并为单一来源或共享流；新结构必须与仓库既有惯例一致，域不搞特例。
  confidence: 3 ｜ evidence: 2026-09-08 OpenSpec 跨平台、09-09 note 瘦指针；09-26 AI 模块夹层被纠正（"别的模块都没有复合"） ｜ last_updated: 2026-09-26
